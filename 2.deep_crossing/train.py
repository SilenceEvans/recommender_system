...
from deep_crossing_model import DeepCrossing
...
def train(epoch):
    ...
    optimizer = Adam(dc_model.parameters(),lr=0.001)
    ...
    torch.save(dc_model.state_dict(), 'models/dc.pkl')
    torch.save(dc_model.state_dict(), 'models/optimizer.pkl')
...
def eval(epoch):
    dc_model = DeepCrossing()
    dc_model.load_state_dict(torch.load('models/dc.pkl'))
...
def plot_metric(metric):
    ...
    plt.title('Training and validation_' + metric)
    plt.xlabel('epochs')
    plt.ylabel(metric)

...
from wide_deep_model import WideDeep
...
def train(epoch):
    ...
    torch.save(model.state_dict(), 'models/wide&deep.pkl')
...
def eval(epoch):
    model = WideDeep()
    model.load_state_dict(torch.load('models/wide&deep.pkl'),strict=False)
...
def plot_metric(metric):
    ...
    plt.title('WideDeep Training and validation_' + metric)
    plt.xlabel('epochs')
    plt.ylabel(metric)
